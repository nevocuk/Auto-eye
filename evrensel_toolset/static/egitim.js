...
document.getElementById("baslat-btn").addEventListener("click", async () => {
  ...
  if (mod === "devam") {
    ...
    // Üçü de boşsa (dosya seçilmedi, isim de yazılmadı) MECBUR ETMİYORUZ --
    // backend zaten hiçbir şey gelmezse "yolo11n.pt" varsayılanını
    // kullanıyor (bkz. app.py `model_yolu = veri.get(...) or "yolo11n.pt"`).
    // Yani bu alan aslında zorunlu değil, sadece varsayılanı değiştirmek
    // isteyenler için var.

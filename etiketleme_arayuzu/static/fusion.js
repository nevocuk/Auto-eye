...
document.getElementById("fusion-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    const satirlar = veri.araclar.map((a) => {
      ...
      const renk = a.renk || "—";
      return `<strong>Araç #${a.arac_no}</strong> — marka/model: ${marka} &nbsp;|&nbsp; renk: ${renk} &nbsp;|&nbsp; plaka: ${plaka}`;
    });
...
// ================= SÜREKLİ İZLEME MODU (video / webcam) =================
let izlemePollingId = null;
let izlemeKaynak = "video"; // "video" | "webcam"

document.querySelectorAll(".izleme-kaynak-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    document.querySelectorAll(".izleme-kaynak-btn").forEach((b) => b.classList.remove("izleme-kaynak-aktif"));
    btn.classList.add("izleme-kaynak-aktif");
    izlemeKaynak = btn.dataset.izlemeKaynak;
    document.getElementById("izleme-video-secim").classList.toggle("gizli", izlemeKaynak !== "video");
  });
});
...
document.getElementById("izleme-baslat-btn").addEventListener("click", async () => {
  ...
  if (izlemeKaynak === "video" && !fusionYollar.izleme_video_dosyasi) { hataEl.textContent = "Önce bir video seç."; return; }
  ...
  const govde = {
    ...
    webcam: izlemeKaynak === "webcam",
  };
  if (izlemeKaynak === "video") govde.video_dosyasi = fusionYollar.izleme_video_dosyasi;
  ...
  const goruntuEl = document.getElementById("izleme-webcam-goruntu");
  if (izlemeKaynak === "webcam") {
    goruntuEl.src = "/api/fusion/izleme_webcam_stream?t=" + Date.now();
    goruntuEl.classList.remove("gizli");
  } else {
    goruntuEl.src = "";
    goruntuEl.classList.add("gizli");
  }

...
# Model Eğitimi/Test/Ön-Etiketleme/Fusion için gereken AĞIR ve modüle
# göre OPSİYONEL bağımlılıkları (ultralytics, opencv-python,
# imageio-ffmpeg, torch, torchvision, fast-plate-ocr, pillow-heif) +
# hazır COCO modelini (yolo11n.pt) tek tuşla kurmak/indirmek için var.
# (evrensel_toolset/'teki aynı özelliğin bu projeye uyarlanmış hali --
# TEK FARK: burada Fusion HER ZAMAN açık olduğu için fast-plate-ocr/
# pillow-heif "opsiyonel" değil "zorunlu" grupta.)
# ============================================================
...
GEREKLILIK_PAKETLERI = [
    ...
    ("ultralytics", "ultralytics", "Model Eğitimi, Model Test, Ön-Etiketleme, Fusion"),
    ("opencv-python", "cv2", "Model Eğitimi, Model Test, Ön-Etiketleme, Video'dan Kare Çıkar, Fusion"),
...
# Bu projede opsiyonel grup yok (Fusion her zaman açık) -- ama
# gereklilikler.html/js diğer kopyayla aynı yapıyı beklediği için boş
# liste olarak tanımlı tutuyoruz.
...
def augment_on_izle():
    ...
    return jsonify({
        # 2026-08-05: "etiketli" adı yanıltıcıydı -- artık etiketsiz
        # görseller de sayılıyor (nevfel.txt'nin istediği images-only girdi
        # modu). Toplam görsel sayısı bu.
...
# gözden geçirilmeli.
#
# Hangi COCO class'larının "araç" sayılacağını class ADINA göre (id'ye
# göre değil) buluyoruz -- böylece hem standart COCO modellerinde hem
# de farklı class sırasına sahip başka modellerde doğru çalışır. Eğer
# hiç "car/truck/bus" bulunamazsa (örn. kendi custom modelini verirsen,
# onun class'ları zaten farklı isimler), hiç filtre uygulamadan en
# güvenilir kutuyu kullanırız.
# ============================================================
...
def _arac_class_idlerini_bul(model) -> list[int]:
    hedefler = {"car", "truck", "bus"}
...
def onetiket_calistir():
    ...
    arac_idleri = _arac_class_idlerini_bul(model)
    ...
    for gorsel_yolu in gorseller:
        ...
        try:
            ...
            if arac_idleri:
                tahmin_kwargs["classes"] = arac_idleri
    ...
    try:
        ...
        if ayarlar.get("mod") == "devam":
            ...
            model.train(
                ...
                workers=ayarlar["workers"],
                cache=ayarlar["cache"],
                cos_lr=ayarlar["cos_lr"],
                lr0=ayarlar["lr0"],
                warmup_epochs=ayarlar["warmup_epochs"],
            )
        ...
        if mod == "devam":
            ...
            try:
                ayarlar = {
                    ...
                    # workers: CPU tarafında görsel okuma/augment yapan paralel
                    # process sayısı. Ultralytics varsayılanı 8 -- eğer CPU
                    # yeterince hızlı veri hazırlayamıyorsa GPU her batch'te
                    # BOŞ BEKLER (VRAM dolu görünür ama fan/kullanım düşük
                    # kalır, "GPU'yu zorlamıyor" izlenimi budur). Daha güçlü
                    # bir CPU'da bu sayıyı artırmak GPU'yu daha sürekli
                    # besler, eğitimi hızlandırır -- ama çekirdek sayısını
                    # aşınca fayda durur/ters teper (context-switch yükü).
                    "workers": int(veri.get("workers", 8)),
                    # cache: görselleri RAM'de ("ram") ya da diskte .npy
                    # olarak ("disk") önbelleğe alır -- her epoch'ta aynı
                    # binlerce küçük jpg'yi yeniden diskten okumak/decode
                    # etmek yerine bir kere okuyup saklar. Yeterli RAM varsa
                    # ("ram") en hızlısı, disk I/O'yu tamamen ortadan kaldırır.
                    "cache": veri.get("cache") if veri.get("cache") in ("ram", "disk") else False,
                    # cos_lr: sabit/lineer yerine kosinüs eğrisiyle azalan
                    # öğrenme oranı -- çok sayıda class'ta (geniş marka/model
                    # kapsamı gibi) eğitimin sonlarına doğru daha yumuşak
                    # yakınsama sağlar, genelde zararı yok.
                    "cos_lr": bool(veri.get("cos_lr", False)),
                    # lr0/warmup_epochs: "başta öğrenme oranını dusuk tut,
                    # pretrained agirliklari bozma" -- fine-tune senaryosunda
                    # (freeze + kucuk/orta veri) dusuk lr0 + yeterli warmup
                    # onerilir. Varsayilanlar ultralytics'in kendi
                    # varsayilanlariyla AYNI (0.01, 3.0) -- kullanici
                    # dokunmazsa davranis degismez.
                    "lr0": float(veri.get("lr0", 0.01)),
                    "warmup_epochs": float(veri.get("warmup_epochs", 3.0)),
                }
            except (TypeError, ValueError):
    ...
    if sonuc.boxes is not None:
        for kutu in sonuc.boxes:
            ...
            tespitler.append({
                ...
                "renk": _renk_tespit_et(sonuc.orig_img, x1, y1, x2, y2),
            })
...
# ============================================================
# MODEL TEST -- KLASÖR (TOPLU) modu
# Etiketsiz bir klasördeki (ör. sahibinden'den indirilmiş fotoğraflar)
# TÜM görselleri tek seferde işleyip, her biri için tahmin kutuları
# çizilmiş küçük resim + tespit listesi döndürür. Metrikler modundan
# farkı: ground-truth etiket İSTEMEZ, sadece gözle kontrol için.
# Senkron çalışır (arka plan thread'i yok) -- makul boyutlu (birkaç
# yüz görsel) klasörler için yeterince hızlı, MAKS_GORSEL üstü reddedilir.
# ============================================================

TEST_KLASOR_UZANTILARI = {".jpg", ".jpeg", ".png", ".bmp", ".webp"}
TEST_KLASOR_MAKS_GORSEL = 500


@app.route("/api/test/klasor", methods=["POST"])
def test_klasor():
    import time as _time
    ...
    veri = request.get_json()
    model_yolu = veri.get("model_dosyasi", "").strip()
    klasor_yolu = veri.get("klasor", "").strip()
    try:
        conf = float(veri.get("conf", 0.25))
    except (TypeError, ValueError):
        conf = 0.25
    ...
    if not model_yolu:
        return jsonify({"hata": "Bir model dosyası seçmelisin (örn. best.pt)."}), 400
    if not klasor_yolu or not Path(klasor_yolu).is_dir():
    ...
    gorseller = sorted(
        p for p in Path(klasor_yolu).iterdir()
        if p.suffix.lower() in TEST_KLASOR_UZANTILARI
    )
    if not gorseller:
        return jsonify({"hata": "Klasörde desteklenen formatta (jpg/png/webp/bmp) görsel bulunamadı."}), 400
    if len(gorseller) > TEST_KLASOR_MAKS_GORSEL:
        return jsonify({
            "hata": f"Klasörde {len(gorseller)} görsel var, tek seferde en fazla "
                    f"{TEST_KLASOR_MAKS_GORSEL} görsel işlenebilir -- daha küçük bir klasörle dene."
        }), 400
    ...
    for gorsel_yolu in gorseller:
        try:
            tahmin = model.predict(source=str(gorsel_yolu), conf=conf, verbose=False)[0]
        except Exception as e:
            sonuclar.append({"dosya_adi": gorsel_yolu.name, "hata": str(e), "tespitler": []})
            continue

        cizili = tahmin.plot()
        dosya_adi = f"toplu_{int(_time.time() * 1000)}_{gorsel_yolu.stem}.jpg"
        cv2.imwrite(str(TEST_CIKTI_KLASORU / dosya_adi), cizili)

        tespitler = []
        if tahmin.boxes is not None:
            for kutu in tahmin.boxes:
                sinif_id = int(kutu.cls[0])
                x1, y1, x2, y2 = map(int, kutu.xyxy[0].tolist())
                tespitler.append({
                    "sinif": tahmin.names.get(sinif_id, str(sinif_id)),
                    "guven": round(float(kutu.conf[0]), 4),
                    "renk": _renk_tespit_et(tahmin.orig_img, x1, y1, x2, y2),
                })

        sonuclar.append({
            "dosya_adi": gorsel_yolu.name,
            "sonuc_url": f"/static/test_ciktilari/{dosya_adi}",
            "tespitler": tespitler,
        })

    return jsonify({"ok": True, "toplam": len(gorseller), "sonuclar": sonuclar})
...
# başındaki not).
FUSION_DB_YOLU = Path(__file__).resolve().parent.parent / "db" / "otogoz.db"
...
def _fusion_db_tablosunu_olustur():
    ...
    with sqlite3.connect(FUSION_DB_YOLU) as con:
        ...
        """)
        # Eski (renk kolonu olmadan oluşturulmuş) db/otogoz.db dosyaları için
        # migration: kolon zaten varsa sqlite hata verir, onu yutuyoruz.
        try:
            con.execute("ALTER TABLE tespitler ADD COLUMN renk TEXT")
        ...
        con.execute("CREATE INDEX IF NOT EXISTS idx_tespitler_renk ON tespitler(renk)")
            ...
            con.execute(
                """INSERT INTO tespitler
                   (tespit_zamani, gorsel_yolu, sonuc_gorsel_yolu,
                    marka_model, marka_model_guven, marka_model_durum, renk,
...
# --- Renk tespiti (eğitimsiz, HSV tabanlı) ---
# Model eğitmiyoruz -- tespit kutusunun içindeki pikselleri HSV renk
# uzayına çevirip en çok tekrar eden tona bakıyoruz. Kutunun TAM kenarları
# yerine ORTA %50'lik bölgesini örnekliyoruz ki tekerlek/cam/gölge/arka
# plan gibi kaporta rengini yansıtmayan pikseller ağırlığı bozmasın.
def _renk_tespit_et(goruntu, x1: int, y1: int, x2: int, y2: int) -> str | None:
    """Bir tespit kutusunun baskın rengini HSV ortalamasına bakarak tahmin eder.
    Eğitim/model GEREKTİRMEZ -- sadece OpenCV renk uzayı dönüşümü + histogram.
    Döndürür: "beyaz", "siyah", "gri", "gümüş", "kırmızı", "mavi", "yeşil",
    "sarı", "turuncu", "mor", "kahverengi" ya da tespit edilemezse None.
    """
    try:
        import cv2
        import numpy as np
    ...
    kutu_genislik, kutu_yukseklik = x2 - x1, y2 - y1
    # orta %50'lik bölge (kenarlardan %25 pay bırak)
    ox1 = x1 + int(kutu_genislik * 0.25)
    ox2 = x2 - int(kutu_genislik * 0.25)
    oy1 = y1 + int(kutu_yukseklik * 0.25)
    oy2 = y2 - int(kutu_yukseklik * 0.25)
    kirpik = goruntu[oy1:oy2, ox1:ox2]
    if kirpik.size == 0:
        kirpik = goruntu[y1:y2, x1:x2]
    if kirpik.size == 0:
        return None

    hsv = cv2.cvtColor(kirpik, cv2.COLOR_BGR2HSV)
    # medyan kullanıyoruz (ortalama değil) -- birkaç aşırı parlak/karanlık
    # piksel (yansıma, gölge) ortalamayı kolayca çarpıtabilir, medyan
    # bunlara karşı daha dayanıklı.
    h = float(np.median(hsv[:, :, 0]))
    s = float(np.median(hsv[:, :, 1]))
    v = float(np.median(hsv[:, :, 2]))

    # 1) Doygunluğu çok düşük olanlar (renksiz eksen: siyah-gri-beyaz)
    if s < 40:
        if v < 60:
            return "siyah"
        if v > 190:
    ...
    # 2) Doygunluğu orta/yüksek olanlar -- hue'ya göre isimlendir
    if h < 8 or h >= 170:
        return "kırmızı"
    if h < 20:
        return "turuncu"
...
def fusion_calistir():
    ...
    # NOT: conf=0.10 ile ÇAĞIRIYORUZ (ultralytics'in kendi varsayılanı
    # 0.25'tir) -- amaç daha fazla aday YAKALAMAK değil, "hiç kutu
    # bulunamadı" ile "düşük güvenli bir aday bulundu ama elendi"
    # durumlarını AYIRT EDEBİLMEK. Asıl kabul eşiği hâlâ
    # FUSION_PLAKA_TESPIT_ESIK_MIN (0.40) -- bu sadece 0.10-0.40 arası
    # "sınırda" adayların "[atlandi]" olarak log'a düşmesini sağlıyor,
    # tamamen görünmez olmalarını değil (teşhis/debug amaçlı).
    plaka_sonuc = plaka_model(goruntu, conf=0.10, verbose=False)[0]
    plaka_kutulari = []
    ...
    if not plaka_sonuc.boxes or len(plaka_sonuc.boxes) == 0:
        log(
            "  [bilgi] Plaka tespit modeli görselde HİÇ aday bulamadı (conf=0.10 ile bile). "
            "Genellikle plaka görsel içinde çok küçük/uzak kaldığında ya da görsel düşük "
            "çözünürlüklü olduğunda (örn. bir galeri/thumbnail'in ekran görüntüsü) olur -- "
            "orijinal, yüksek çözünürlüklü fotoğrafla ya da plakaya daha yakın bir kırpıkla "
            "tekrar denemek genelde çözer."
        )
    ...
    for i, (class_adi, guven, durum) in enumerate(arac_tahminleri):
        ...
        araclar.append({
            ...
            "renk": _renk_tespit_et(goruntu, x1, y1, x2, y2),
            "plaka": None,
            "plaka_guven": None,
    ...
    for arac in araclar:
        log(
            ...
            f"renk={arac.get('renk') or '(belirlenemedi)'}  |  "
    ...
    kayitlar_klasoru = Path(__file__).resolve().parent.parent / "kayitlar"
...
IZLEME_DURUMU = {
    ...
    "loglar": [],
    "webcam_mi": False,
    "son_kare_jpeg": None,  # canlı önizleme akışı (webcam modu) için
}
...
def _izleme_dongusu(marka_model_yolu: str, plaka_model_yolu: str, video_yolu: str, conf: float, webcam_mi: bool = False):
    import time as _time
    try:
        import cv2
    except ImportError:
    ...
    try:
        ...
        if webcam_mi:
            # Model Test'teki webcam mantığıyla aynı: DSHOW backend'i
            # Windows'ta "kamera açık görünüp hiç kare vermeme" sorununu
            # genelde çözüyor, önce onu deniyoruz.
            acilan = cv2.VideoCapture(0, cv2.CAP_DSHOW)
            if not acilan.isOpened():
                acilan = cv2.VideoCapture(0)
            if not acilan.isOpened():
                raise RuntimeError(
                    "Webcam açılamadı -- başka bir uygulama (Zoom, Teams vb.) "
                    "kullanıyor olabilir, bilgisayarda webcam yok, ya da Windows "
            ...
            IZLEME_DURUMU["toplam_kare"] = 0  # webcam'de sabit bir toplam yok, canlı akış
        else:
            acilan = cv2.VideoCapture(video_yolu)
            if not acilan.isOpened():
                raise RuntimeError(f"Video açılamadı: {video_yolu}")
        ...
        def _finalize_et(track_id):
            ...
            try:
                ...
                _izleme_log(
                    ...
                    f"renk={kayit.get('renk') or '(belirlenemedi)'}, "
        ...
        islenen = 0
        ardarda_basarisiz = 0
        MAKSIMUM_ARDARDA_BASARISIZ = 100  # ~birkaç saniye -- sadece webcam modunda kullanılıyor
        while not IZLEME_DURDURMA_ISARETI.is_set():
            basarili, kare = acilan.read()
            if not basarili:
                if webcam_mi:
                    # Video dosyasında "kare gelmedi" = dosya bitti demek
                    # (döngüden çık), ama webcam'de bu genelde GEÇİCİ bir
                    # okuma aksaklığı -- Model Test'teki webcam mantığıyla
                    # aynı: kısa süre dene, çok uzun sürerse gerçek hata say.
                    ardarda_basarisiz += 1
                    if ardarda_basarisiz >= MAKSIMUM_ARDARDA_BASARISIZ:
                        raise RuntimeError(
                            "Webcam açıldı ama hiç görüntü alınamadı -- muhtemelen "
                            "Windows Ayarlar > Gizlilik ve Güvenlik > Kamera altında "
                            "bu uygulamaya izin verilmemiş. Ayarı açıp tekrar deneyin."
                        )
                    _time.sleep(0.05)
                    continue
                break
            ...
            if sonuc.boxes is not None and sonuc.boxes.id is not None:
                for kutu in sonuc.boxes:
                    ...
                    # (2026-08-07, kullanıcı geri bildirimiyle düzeltildi).
                    #
                    ...
                    if gorsel_guncellenmeli:
                        ...
                        # Renk de AYNI karede (en iyi/en net görünüm) hesaplanır
                        # -- görsel hangi kareye donduysa renk de o karedeki
                        # kaporta rengini yansıtsın diye.
                        kayit_ref["renk"] = _renk_tespit_et(kare, x1, y1, x2, y2)
            ...
            # Canlı önizleme akışı (webcam modunda tarayıcıda gösteriliyor,
            # video-dosyası modunda da zararsız -- sadece kimse izlemiyorsa
            # kullanılmıyor). Şu ana kadarki EN İYİ bilinen sınıf/plaka
            # bilgisiyle güncel kutuları çiziyor -- kaydedilecek "en iyi
            # kare" mantığından bağımsız, sadece anlık görüntü.
            canli_kare = kare.copy()
            for tid, (ckutu, _cclass, _cguven) in gorulen_bu_karede.items():
                ckayit = bekleyenler[tid]
                cx1, cy1, cx2, cy2 = ckutu
                cv2.rectangle(canli_kare, (cx1, cy1), (cx2, cy2), (0, 255, 0), 2)
                cetiket = f"#{tid} {ckayit['class_adi']}"
                if ckayit.get("plaka"):
                    cetiket += f" | {ckayit['plaka']}"
                cv2.putText(canli_kare, cetiket, (cx1, max(0, cy1 - 8)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            basarili_jpeg, jpeg_veri = cv2.imencode(".jpg", canli_kare)
            if basarili_jpeg:
                with IZLEME_KILIDI:
                    IZLEME_DURUMU["son_kare_jpeg"] = jpeg_veri.tobytes()
...
def fusion_izleme_baslat():
    with IZLEME_KILIDI:
        ...
        marka_model_yolu = veri.get("marka_model_dosyasi", "").strip()
        plaka_model_yolu = veri.get("plaka_model_dosyasi", "").strip()
        webcam_mi = bool(veri.get("webcam", False))
        video_yolu = veri.get("video_dosyasi", "").strip()
        try:
            conf = float(veri.get("conf", 0.25))
        except (TypeError, ValueError):
        ...
        if not webcam_mi and (not video_yolu or not Path(video_yolu).exists()):
            return jsonify({"hata": f"Video bulunamadı: {video_yolu}"}), 400
        ...
        thread = threading.Thread(
            ...
            args=(marka_model_yolu, plaka_model_yolu, video_yolu, conf, webcam_mi),
            daemon=True,
        )
        thread.start()
...
def fusion_izleme_durum():
    with IZLEME_KILIDI:
        ...
        kopya.pop("son_kare_jpeg", None)  # binary veri -- JSON'a gerek yok, ayrı stream route'u var
    return jsonify(kopya)


@app.route("/api/fusion/izleme_webcam_stream")
def fusion_izleme_webcam_stream():
    """İzleme webcam modundayken canlı (MJPEG) önizleme akışı --
    Model Test'teki webcam_stream ile aynı desen."""
    import time as _time

    def kare_uret():
        bekleme = 0
        while IZLEME_DURUMU["son_kare_jpeg"] is None and IZLEME_DURUMU["calisiyor"] and bekleme < 50:
            _time.sleep(0.1)
            bekleme += 1

        while IZLEME_DURUMU["calisiyor"]:
            with IZLEME_KILIDI:
                jpeg_veri = IZLEME_DURUMU["son_kare_jpeg"]
            if jpeg_veri is not None:
                yield (b"--frame\r\nContent-Type: image/jpeg\r\n\r\n" + jpeg_veri + b"\r\n")
            _time.sleep(0.05)
...
# ============================================================
# VERİTABANI SORGU (db/otogoz.db "tespitler" tablosunu arama/
# gözden geçirme arayüzü -- Fusion/Sürekli İzleme çalışırken
# biriken kayıtları marka/model, plaka ya da tarihe göre aramak
# için. Not: veritabanında aracın ÜRETİM YILI tutulmuyor -- sadece
# "tespit_zamani" (o kaydın NE ZAMAN tespit edildiği) var. "Yıl"
# filtresi bu yüzden tespit_zamani'nin yıl kısmına göre çalışıyor,
# aracın model yılına göre değil (bkz. kod/07_veritabani.py'deki
# şema kararı notları).
# ============================================================

@app.route("/veritabani")
def veritabani_sayfasi():
    return render_template("veritabani.html")


def _veritabani_gorsel_url(dosya_yolu: str):
    """DB'de tutulan MUTLAK dosya yolunu, static/fusion_ciktilari
    altındaysa tarayıcıdan erişilebilir bir /static/... URL'sine
    çevirir. Klasörün dışındaysa (ör. eski/taşınmış kayıt) None döner
    -- arayüz o zaman sadece yolu metin olarak gösterir, kırık resim
    göstermez."""
    if not dosya_yolu:
        return None
    try:
        tam = Path(dosya_yolu).resolve()
        kok = FUSION_CIKTI_KLASORU.resolve()
        if kok in tam.parents or tam == kok:
            return f"/static/fusion_ciktilari/{tam.relative_to(kok).as_posix()}"
    except (OSError, ValueError):
        pass
    return None


@app.route("/api/veritabani/ara")
def veritabani_ara():
    import sqlite3

    # Geriye dönük uyum: eski "q" tek kutu parametresi hâlâ gelirse
    # marka_model VEYA plaka içinde arar. Yeni arayüz artık iki ayrı
    # kutu kullanıyor: marka_model ve plaka, ayrı ayrı (AND ile) filtreler.
    q = (request.args.get("q") or "").strip()
    marka_model_q = (request.args.get("marka_model") or "").strip()
    plaka_q = (request.args.get("plaka") or "").strip()
    renk_q = (request.args.get("renk") or "").strip()
    yil = (request.args.get("yil") or "").strip()
    ...
    if q:
        kosullar.append("(marka_model LIKE ? COLLATE NOCASE OR plaka LIKE ? COLLATE NOCASE)")
        parametreler.extend([f"%{q}%", f"%{q}%"])

    if marka_model_q:
        # büyük/küçük harf duyarsız, kısmi eşleşme -- "egea" tek başına da
        # "fiat_egea" gibi bir değer içinde bulunur (LIKE %...% substring).
        kosullar.append("marka_model LIKE ? COLLATE NOCASE")
        parametreler.append(f"%{marka_model_q}%")

    if plaka_q:
        # kullanıcı boşluk/tire ile yazabilir (ör. "34 ABC 123"); DB'de
        # genelde bitişik saklanıyor, aramadan önce boşluk/tireyi at.
        plaka_temiz = re.sub(r"[\s-]+", "", plaka_q)
        kosullar.append("REPLACE(REPLACE(plaka, ' ', ''), '-', '') LIKE ? COLLATE NOCASE")
        parametreler.append(f"%{plaka_temiz}%")

    if renk_q:
        kosullar.append("renk LIKE ? COLLATE NOCASE")
        parametreler.append(f"%{renk_q}%")

    if yil:
        # tespit_zamani ISO formatında ("2026-08-06T10:15:00") saklanıyor,
        # yıl ile başlayıp başlamadığına bakmak yeterli
        kosullar.append("tespit_zamani LIKE ?")
        parametreler.append(f"{yil}%")

    sorgu = "SELECT * FROM tespitler"
    if kosullar:
        sorgu += " WHERE " + " AND ".join(kosullar)
    sorgu += " ORDER BY tespit_zamani DESC LIMIT ?"
    parametreler.append(limit)

    with sqlite3.connect(FUSION_DB_YOLU) as con:
        con.row_factory = sqlite3.Row
        satirlar = con.execute(sorgu, parametreler).fetchall()
        toplam_kayit = con.execute("SELECT COUNT(*) FROM tespitler").fetchone()[0]

    sonuclar = []
    for s in satirlar:
        d = dict(s)
        d["gorsel_url"] = _veritabani_gorsel_url(d.get("sonuc_gorsel_yolu") or d.get("gorsel_yolu"))
        sonuclar.append(d)

    return jsonify({
        "kayitlar": sonuclar,
        "gosterilen": len(sonuclar),
        "toplam_kayit_veritabaninda": toplam_kayit,
    })
